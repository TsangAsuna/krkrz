...
struct tTVPFileInfo
{
	...
	tTVPFileInfo( const ttstr& path, const ttstr& name,
		tjs_int flag = 0, iTJSDispatch2* prop = nullptr )
	: FilePath(path), FileName(name), Property(prop), Flag(flag)
	{
		if( Property ) Property->AddRef();
	}
	tTVPFileInfo( const tTVPFileInfo& info )
	: FilePath(info.FilePath), FileName(info.FileName),
		Property(info.Property), Flag(info.Flag )
	{
		if( Property ) Property->AddRef();
	}
	~tTVPFileInfo()
			...
			if( !TVPIgnoreFileProperty )
			{
				...
				for( auto i = lister.prop.begin(); i != lister.prop.end(); i++ ) {
					...
					if( file != lister.list.end() ) {
						ttstr actualname = *file;
						ttstr sname = actualname;
						sname.ToLowerCase();
						// ファイルがある場合
						lister.list.erase( file );
						TVPAutoPathTable.Add( sname,
							tTVPFileInfo( path, actualname, tTVPFileInfo::EXIST_PROP ) );
					} else {
						ttstr sname = fname;
						sname.ToLowerCase();
						TVPAutoPathTable.Add( sname,
							tTVPFileInfo( path, fname,
								tTVPFileInfo::EXIST_PROP | tTVPFileInfo::EMPTY_FILE ) );
					}
				}
			}
			// プロパティのないファイルを追加する
...
ttstr TVPGetPlacedPath(const ttstr & name)
{
	...
	{
		// found in table
		ttstr found = result->FilePath + result->FileName;
...
iTJSDispatch2* TVPGetFilePropertyNoAddRef( const ttstr& name )
{
	...
	if( result && ( result->Flag & tTVPFileInfo::EXIST_PROP) ) {
		// found in table
		if( !result->Property ) {
			ttstr path = result->FilePath + result->FileName + ".prop";
			tTJSVariant dic;
			ttstr mode;
