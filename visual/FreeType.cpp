...
tBaseFreeTypeFace* tTVPFreeTypeFaceList::GetFace( const tjs_string& facename, tjs_uint32 options ) const {
	...
	// System fonts must remain available even after an asset font is loaded.
	// Prefer an exact style, then fall back to the family so FreeType can
	// synthesize bold/italic when Android only ships a Regular face.
	...
	auto systemface = std::find_if(systemfaces_.begin(), systemfaces_.end(),
		[facename]( const FontInfo* font ) {
			return font->facename == facename ||
				std::find(font->aliases.begin(), font->aliases.end(), facename) != font->aliases.end();
		});
	if( systemface != systemfaces_.end() ) {
		FontInfo* font = *systemface;
		return new tGenericFreeTypeFace( font->path, ttstr( facename ), font->index );
	}
	return nullptr;
}
