...
{
	GetCurrentRasterizer()->ApplyFont( GetFont() );
	tjs_uint width = 0;
...
{
	if( Layer && Layer->GetMainImage() ) return Layer->GetTextWidth(text);
	return GetTextWidthDirect(text);
}
//---------------------------------------------------------------------------
tjs_int tTJSNI_Font::GetTextHeight(const ttstr & text)
{
	if( Layer && Layer->GetMainImage() ) return Layer->GetTextHeight(text);
	return std::abs(GetFont().Height);
}
//---------------------------------------------------------------------------
double tTJSNI_Font::GetEscWidthX(const ttstr & text)
{
	if( Layer && Layer->GetMainImage() ) return Layer->GetEscWidthX(text);
	return std::cos(GetFont().Angle * (M_PI/1800)) * GetTextWidthDirect(text);
}
//---------------------------------------------------------------------------
double tTJSNI_Font::GetEscWidthY(const ttstr & text)
{
	if( Layer && Layer->GetMainImage() ) return Layer->GetEscWidthY(text);
	return std::sin(GetFont().Angle * (M_PI/1800)) * (-GetTextWidthDirect(text));
}
//---------------------------------------------------------------------------
double tTJSNI_Font::GetEscHeightX(const ttstr & text)
{
	if( Layer && Layer->GetMainImage() ) return Layer->GetEscHeightX(text);
	return std::sin(GetFont().Angle * (M_PI/1800)) * std::abs(GetFont().Height);
}
//---------------------------------------------------------------------------
double tTJSNI_Font::GetEscHeightY(const ttstr & text)
{
	if( Layer && Layer->GetMainImage() ) return Layer->GetEscHeightY(text);
	return std::cos(GetFont().Angle * (M_PI/1800)) * std::abs(GetFont().Height);
}
//---------------------------------------------------------------------------
